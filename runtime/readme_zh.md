...
```shell
cd /path/to/EchoCore/runtime
mkdir build && cd build
cmake -DCMAKE_BUILD_TYPE=release .. \
  -DONNXRUNTIME_DIR=/path/to/EchoCore/runtime/onnxruntime-linux-x64-1.14.0 \
  -DFFMPEG_DIR=/path/to/EchoCore/runtime/ffmpeg-master-latest-linux64-gpl-shared \
  -DGPU=ON
make -j 4
```
...
```
Visual Studio 打开 `EchoCore/runtime/build/FunASRWebscoket.sln` 完成编译；
编译后的可执行文件位于：`EchoCore/runtime/build/bin/Debug`；
从 `onnxruntime-win-x64-1.16.1/lib`、`ffmpeg-master-latest-win64-gpl-shared/bin`、`openssl-1.1.1w/bin` copy 相关 DLL 到 `EchoCore/runtime/build/bin/Debug`
